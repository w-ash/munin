...
```yaml
type: museum              # museum | gallery | theater | park | garden | temple | cultural | immersive | nightlife | festival | fireworks | walk | tour | ride | shopping
focus: contemporary-art   # contemporary-art | design | photography | craft | traditional | media-art | folk-art | architecture | cinema | ukiyo-e | food-tour | bar-hopping | jazz | sport | stroll | hike | food-market | tea-sweets | cycling | shrine | zen | jodo | shinto | animation
hours: ""
closed: ""
admission: ""
...
- **temple** — Buddhist temples + Shinto shrines (combined under one type — `focus: shinto/zen/jodo` differentiates).
- **garden** — designed gardens (Murin-an, Saihō-ji moss, Tenryū-ji Sōgenchi). Strolls *through* a garden as the experience belong here, not under `walk`.
...
- **immersive** — site-specific or sensory installations (teamLab, projection-mapping, immersive theatre).
...
- **ride** — scenic vehicle journeys where the journey itself is the experience: trolleys (Sagano), gondolas (Yokohama Air Cabin), cable cars / chair lifts (Mt Takao), water buses (Hotaluna), helicopter night flights, scenic river boats (Hozugawa). Narration onboard doesn't promote a ride to a tour — the test is whether a guide is leading you somewhere on foot or whether the vehicle is the experience.
- **theater** — kabuki, Takarazuka, Noh, ballet, butoh, etc.
- **nightlife** — clubs, listening bars, late-night cultural venues.
- **festival / fireworks** — date-bounded annual events.
- **shopping** — mall / retail-street / district experiences (Kappabashi kitchen-equipment street, Nakano Broadway, Shibuya Scramble Square). Distinct from `Travel/<Trip>/Shopping/` which catalogs **individual shops** to buy specific things. **Depachika department stores** (Isetan, Mitsukoshi, Takashimaya, Matsuya, Daimaru) live in `Travel/<Trip>/Shopping/` as `type: department`, not here — even though the architecture and food-hall floor are experience-grade, the venue is structurally a shop.
...
`tier` is on the shared frontmatter — see the **Tier list** section in `.claude/rules/travel.md` for definitions and the **Sourcing gate** that gates SS/S. Bucket by `type` (museum, garden, temple, walk, tour, etc.). Experiences-specific tier signals will be added once we've learned from the Tokyo dining tier pass.
...
- **Skip contact fighting sports** in must-do / cultural-experience / activity recommendations. Sumo, boxing, MMA, wrestling, etc. — the welfare-grounded objection is to physical-contact combat itself, not to whether it's packaged as ceremony or cultural tradition. Day-of walk-up is fine if Ash brings it up; don't propose it.
...
- **Language-accessibility filter.** When recommending guided experiences in non-English destinations, confirm English support before filing. Three viable shapes:
  1. **Group English tour** with scheduled departure (Kyoto Localized free walks, Context Travel scholar-led, etc.)
  2. **Audio-guide / captioning service** at venues that don't tour-format (Kabukiza captions, museum audio guides)
  3. **Audio-visual / language-transcending experience** — visual / spatial / sensory media where language isn't load-bearing (photography festivals, kabuki / butoh / dance, light installations, immersive art, architecture-as-experience, gardens, projection, sound art). Bilingual wall text is the standard at international-grade Japanese venues for international shows; visual performance traditions don't require language.
  
  Disqualifying without flag: Japanese-only guided tours (e.g. the May 2026 Tokyo Architecture Festival — 119 of 120 paid guide tours were Japanese-only, English option's lottery already closed). When a major-sounding event fails the language filter, surface alternatives that hit the same intent (e.g., year-round group / English architecture walks; private-tour operators when group isn't available; or bilingual museum exhibitions covering similar subject matter).

## Nightlife — venue selection

The reference frame for `type: nightlife` is **music-first rooms**, not "the biggest club" or "Tokyo's most famous." The SF analog set Ash uses: **Public Works / Monarch / Great Northern / 1015 Folsom** — places where the people who love the music go, not the people who just want to get drunk. Apply the same filter for any destination.
...
- Listed in domain editorial: Time Out (city) "best clubs by music genre," RA Guide, Mixmag city guides, DEEPTOKYO equivalent
...
**Bigger rooms are a special case** — venues like Womb (Tokyo) or Printworks-class rooms book serious music *some* nights and tourist-EDM *other* nights. The right filter is the **promoter, not the venue**: a Mods / Future Terror / DBS night at Womb is in the music-first bucket; a "Womb Adventure" / commercial-tour night is not. Always verify the night, not just the room.

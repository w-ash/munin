...
name: ""
name_jp: ""                  # local-language name (non-English destinations)
neighborhood: ""             # quoted Obsidian wikilink to a single neighborhood file: "[[Marylebone]]"; see `.claude/rules/neighborhoods.md`
nearest_station: ""          # station name only, e.g. "Baker Street"
walk_time_to_station:        # integer minutes (null if unknown / not walkable)
station_lines: ""            # comma-separated, e.g. "Bakerloo, Jubilee"
destination: ""              # quoted Obsidian wikilink to the parent destination file: "[[London]]"; must resolve to Travel/<Trip>/Destinations/entries/
...
- **Experiences**: bucket by `type` (`museum`, `gallery`, `garden`, `nightlife`, etc.). Don't sub-bucket by `focus` unless a single type contains genuinely incomparable sub-genres.
...
2. **≥1 citation in the file's Sources section** from a category-recognized reputable editorial; the curated source lists in each category rule (`dining.md` "Curated sources", and the equivalent in `experiences.md` / `shopping.md` / `accommodations.md`) are the canonical "reputable" reference. Examples for dining: Sprudge, a city magazine's best-of list, a national newspaper's food section, a respected local food blog.
3. **A recognized industry badge**: Michelin star or Bib Gourmand, OAD top-100, the 50 Best lists, Sprudge "Notable Roaster," etc.

**Disqualifiers**: these alone are *not* enough to reach S/SS, no matter how compelling the writeup sounds:
- Aggregator-only sourcing: Google Maps rating, TripAdvisor rank, Yelp, Foursquare
- Generic SEO travel blogs: going-awesome-places, Will Fly for Food, content-mill "top 10" listicles, and the like
...
- **`neighborhood:` is always a quoted wikilink** (`"[[Marylebone]]"`) to a file in `Travel/<Trip>/Neighborhoods/entries/`. Never plaintext, never compound (`X / Y`), never a list. Create a neighborhood stub first if the target doesn't exist; see `.claude/rules/neighborhoods.md`.
- **`destination:` is always a quoted wikilink** (`"[[London]]"`) to a file in `Travel/<Trip>/Destinations/entries/`. Never plaintext. Same applies to neighborhood files themselves; they wikilink up to their parent destination.
...
- **Wake times start at 7:00 AM or later.** Never propose a pre-dawn (roughly 5:00–6:30 AM) start as the default day plan. Offer dawn crowd-avoidance only as an explicit alternative with the tradeoff stated.
- **Visited places file without re-litigation.** Once Ash has been somewhere, file it with the status that records the visit (`chosen` for destinations, `visited` for option files) and document what he experienced; the tier and sourcing filters govern prospective planning only.
- **Documented risk outranks optimism.** Before encouraging a booking or purchase, surface any risk the vault already records for that vendor or route (payment 3DS failures, visa or timing gotchas) rather than an upbeat go-ahead.
